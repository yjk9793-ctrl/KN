...
3. 공개 사이트에서는 `/api/listBoardPosts`로 게시글 목록을, `/api/getBoardPost`로 상세 내용을 확인하며 최신 글은 72시간 동안 “New” 배지로 상단에 노출됩니다.
   - 상단 통계 카드에서 전체 게시글/신규 게시글/조회수를 실시간으로 확인하고, 검색창과 “전체/최신” 필터 버튼으로 목록을 즉시 필터링할 수 있습니다.
   - 목록은 번호·제목·작성자·작성일·조회수를 포함한 테이블 형태이며, 제목을 클릭하면 같은 페이지 하단 카드에서 본문·첨부·댓글을 확인할 수 있습니다.
   - 상세 조회 시 조회수가 자동 증가하며 테이블과 통계 카드에도 즉시 반영됩니다.
